...
async def read_all(data_base: Session = Depends(get_db)):
    ...
    return data_base.query(app.models.Users).all()
...
async def get_user_by_path(user_id: int, data_base: Session = Depends(get_db)):
    ...
    user_model = (
        data_base.query(app.models.Users).filter(app.models.Users.id == user_id).first()
    )
...
async def user_password_change(
    ...
    user_model = (
        data_base.query(app.models.Users).filter(app.models.Users.id == user.get("id")).first()
    )
...
async def delete_user(user: dict = Depends(get_current_user), data_base: Session = Depends(get_db)):
    ...
    user_model = data_base.query(app.models.Users).filter(app.models.Users.id == user.get("id")).first()

    if user_model is None:
        return "Invalid user or request"

    data_base.query(app.models.Users).filter(app.models.Users.id == user.get("id")).delete()
    data_base.commit()
